> just copy for self use. please do not use for prod. thanks

| -      | -                                          |
| :----- | :----------------------------------------- |
| author | Dom Christie                               |
| repo   | https://github.com/mixmark-io/turndown.git |

---

## Addon feature

1. use `vite` instand `rollup` for build.

   ```js
   if (process.browser) // remove

   if (typeof window !== undefined) // instand
   ```

2. support `video` tag

   just keep video origin info to markdown

3. code lang support data-language

   some code block use data-language attr for info
